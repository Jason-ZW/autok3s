...
func (p *K3d) Rollback() error {
	return p.RollbackCluster(p.rollbackK3d)
}
...
func (p *K3d) CreateCheck() error {
	...
	if p.ServersMemory != "" {
		if _, err := dockerunits.RAMInBytes(p.ServersMemory); err != nil {
			return fmt.Errorf("[%s] calling preflight error: provided `--servers-memory` limit value is invalid", p.GetProviderName())
		}
	}
	if p.AgentsMemory != "" {
		if _, err := dockerunits.RAMInBytes(p.AgentsMemory); err != nil {
			return fmt.Errorf("[%s] calling preflight error: provided `--agents-memory` limit value is invalid", p.GetProviderName())
		}
	}
...
func (p *K3d) createK3d(ssh *types.SSH) (*types.Cluster, error) {
	...
	cfg, err := p.wrapFlags(masterNum, workerNum)
	if err != nil {
		return nil, err
	}
...
func (p *K3d) joinK3d(ssh *types.SSH) (*types.Cluster, error) {
	var masterNum, _ = strconv.Atoi(p.Master)
	workerNum, _ := strconv.Atoi(p.Worker)
...
func (p *K3d) attachNode(id string, cluster *types.Cluster) error {
	...
	// open dialer tunnel.
	tunnel, err := dialer.OpenTunnel(context.Background(), false, nil, id)
	if err != nil {
		return err
	}
...
// Borrowed from https://github.com/rancher/k3d/blob/main/cmd/cluster/clusterCreate.go#L356.
func (p *K3d) wrapFlags(masters, workers int) (*conf.ClusterConfig, error) {
	ipPorts := strings.Split(p.APIPort, ":")
	...
	if p.ServersMemory != "" {
		cfg.Options.Runtime.ServersMemory = p.ServersMemory
	}

	if p.AgentsMemory != "" {
		cfg.Options.Runtime.AgentsMemory = p.AgentsMemory
	}
	...
	if len(p.Volumes) > 0 {
		...
		for _, volumeFlag := range p.Volumes {
			...
			volume, filter, err := cliutil.SplitFiltersFromFlag(volumeFlag)
			if err != nil {
			...
			if _, exists := volumeFilterMap[volume]; exists {
				volumeFilterMap[volume] = append(volumeFilterMap[volume], filter...)
			} else {
	...
	if p.Ports != "" {
		...
		for _, portFlag := range strings.Split(p.Ports, ",") {
			// split node filter from the specified volume.
			portMap, filter, err := cliutil.SplitFiltersFromFlag(portFlag)
			if err != nil {
	...
	if len(p.Envs) > 0 {
		...
		for _, envFlag := range p.Envs {
			...
			env, filter, err := cliutil.SplitFiltersFromFlag(envFlag)
			if err != nil {
			...
			if _, exists := envFilterMap[env]; exists {
				envFilterMap[env] = append(envFilterMap[env], filter...)
			} else {
				envFilterMap[env] = filter
			}
		}
...
// forwardLogs redirect logs to the cluster's log file,
// will be reconstructed when k3d supports set logrus output.
func (p *K3d) forwardLogs() {
	logrus.SetOutput(p.Logger.Out)
}

// resetLogs reset logrus output to the default,
// will be reconstructed when k3d supports set logrus output.
func (p *K3d) resetLogs() {
