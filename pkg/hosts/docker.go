...
// Terminal open docker terminal.
func (d *DockerDialer) Terminal() error {
	defer func() {
		_ = d.Close()
	}()

	if err := d.ExecStart(); err != nil {
		return err
	}
...
func (d *DockerDialer) ExecStart() error {
	restoreInput, err := d.setInput()
	if err != nil {
...
func (d *DockerDialer) setInput() (restore func(), err error) {
	if d.Stdin == nil {
	...
	go func() {
		if d.Stdin != nil {
			...
			if err != nil {
				...
				logrus.Errorf("[docker-dialer] error send Stdin: %s", err)
			}
		}

		if err := d.response.CloseWrite(); err != nil {
			logrus.Errorf("[docker-dialer] couldn't send EOF: %s", err)
		}
