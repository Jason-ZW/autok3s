...
func (d *WebSocketDialer) Terminal() error {
	switch d.kind {
	case SSHKind:
		return d.sshDialer.Terminal()
	case DockerKind:
		return d.dockerDialer.ExecStart()
	case PtyKind:
		return d.ptyDialer.Terminal()
	}
	return nil
}
...
func (d *WebSocketDialer) ReadMessage(ctx context.Context) error {
	switch d.kind {
	case SSHKind:
		return readMessage(ctx, d.conn, d.Close, d.sshDialer.session.Wait, d.reader.ClosedCh)
	case DockerKind:
...
func (d *WebSocketDialer) ChangeWindowSize(win *WindowSize) {
	...
	switch d.kind {
	case SSHKind:
		err = d.sshDialer.session.WindowChange(win.Height, win.Width)
	case DockerKind:
