...
func (p *ProviderBase) execute(node *types.Node, cmds []string) (string, error) {
	if len(cmds) <= 0 {
		return "", nil
	}
	...
	dialer.SetWriter(p.Logger.Out).SetStdio(&stdout, &stderr, nil)
...
func terminal(node *types.Node) error {
	dialer, err := hosts.NewSSHDialer(node, false)
	if err != nil {
		return err
	}
	...
	dialer.SetStdio(stdout, stderr, stdin)
...
func (p *ProviderBase) handleRegistry(node *types.Node, c *types.Cluster) (err error) {
	cmd := make([]string, 0)
